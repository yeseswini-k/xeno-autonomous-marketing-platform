...
const SegmentBuilder: React.FC<SegmentBuilderProps> = ({ onCampaignCreated, showToast, asOf }) => {
  ...
  const fetchCampaigns = async () => {
    try {
      const res = await fetch(`${API}/api/crm/campaigns?asOf=${asOf}`);
      if (res.ok) {
  ...
  const fetchPreview = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API}/api/crm/segments/preview?asOf=${asOf}`, {
        method: 'POST',
  ...
  const handleNlpCompile = async () => {
    ...
    try {
      const response = await fetch(`${API}/api/crm/ai-prompt?asOf=${asOf}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
